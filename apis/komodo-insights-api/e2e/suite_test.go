...
// Package e2e_test contains end-to-end tests for komodo-insights-api.
// Tests exercise the full request path: HTTP → handler → LLM provider.
//
// Prerequisites:
//   - Run `just up api` to start LocalStack + the service via docker-compose.
//   - Set LLM_API_KEY in LocalStack secrets to enable provider-dependent tests.
//
// Run:
//
...
// Override target URL:
//
//	BASE_URL=http://localhost:7111 go test -tags=e2e -v ./e2e/
package e2e_test
...
func TestMain(m *testing.M) {
	...
	client = &http.Client{Timeout: 30 * time.Second} // elevated for LLM latency
	os.Exit(m.Run())
}
