...
Internal event bus. Accepts domain events from producer services via HTTP and fans them out via AWS SNS + SQS. Also houses the CDC Lambda handler for DynamoDB stream processing.
...
**Deferred.** AWS target when deployed: SNS FIFO (one topic per domain) + SQS FIFO (per-consumer queue with DLQ). This matches the existing EC2/Fargate stack with no new infrastructure primitives. For local dev, runs as a standard HTTP server with in-process fan-out.
...
```
Producer service
  → POST /events (event-bus-api)        ← HTTP path
  OR DynamoDB Stream → CDC Lambda       ← stream path
      → SNS FIFO topic: komodo-<domain>-events-<env>
          → SQS FIFO queue per consumer (komodo-<domain>-events-<consumer>-<env>)
              → DLQ for each queue (retry 3x, then dead-letter)
          → Consumer Lambda / ECS service polls its own queue
```

**Topic naming:** `komodo-order-events-prod`
**Queue naming:** `komodo-order-events-inventory-prod`
**DLQ naming:** `komodo-order-events-inventory-prod-dlq`
...
|---------------------|----------|-------------|
| `APP_NAME`          | Yes | `komodo-event-bus-api` |
