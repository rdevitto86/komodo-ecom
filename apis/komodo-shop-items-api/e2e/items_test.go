...
func TestGetItemBySKU_NotFound(t *testing.T) {
	resp := get(t, "/item/SKU-DOES-NOT-EXIST-E2E", nil)
	defer resp.Body.Close()
...
func TestGetItemBySKU_Valid(t *testing.T) {
	resp := get(t, "/item/TEST-SKU-E2E", nil)
	defer resp.Body.Close()
...
func TestGetSuggestions_RequiresAuth(t *testing.T) {
	resp := post(t, "/item/suggestion", map[string]any{"limit": 6}, nil)
	defer resp.Body.Close()
...
func TestGetSuggestions_Authenticated(t *testing.T) {
	resp := post(t, "/item/suggestion", map[string]any{"limit": 6}, authHeader(t))
	defer resp.Body.Close()
	checkStatus(t, resp, http.StatusOK)
}
...
func TestGetSuggestions_DefaultLimit(t *testing.T) {
	resp := post(t, "/item/suggestion", map[string]any{}, authHeader(t))
	defer resp.Body.Close()
	checkStatus(t, resp, http.StatusOK)
}
