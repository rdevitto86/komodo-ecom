...
import (
	...
	"komodo-cart-api/internal/config"
...
func init() {
	...
	smCfg := awsSM.Config{
		...
		Keys: []string{
			...
			config.DYNAMODB_CARTS_TABLE,
			config.DYNAMODB_ACCESS_KEY,
			config.DYNAMODB_SECRET_KEY,
			config.DYNAMODB_ENDPOINT,
			config.INVENTORY_API_URL,
			config.SHOP_ITEMS_API_URL,
			config.CART_GUEST_TTL_SEC,
			config.CART_HOLD_TTL_SEC,
...
func main() {
	// Wire services.
	guestTTL    := mustParseInt64(os.Getenv(config.CART_GUEST_TTL_SEC), 604800)
	checkoutTTL := mustParseInt64(os.Getenv(config.CART_HOLD_TTL_SEC), 900)
	shopCli     := shopitems.NewClient(os.Getenv(config.SHOP_ITEMS_API_URL))
	invCli      := shopinventory.NewClient(os.Getenv(config.INVENTORY_API_URL))
