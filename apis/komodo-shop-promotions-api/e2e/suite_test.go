...
// Package e2e_test contains end-to-end tests for komodo-shop-promotions-api.
// Tests exercise the full request path: HTTP → handler → DynamoDB (LocalStack).
//
// Prerequisites:
//   - Run `just up api` to start LocalStack + the service via docker-compose.
//   - Set KOMODO_PROMOTIONS_API_URL in LocalStack secrets to enable service-dependent tests.
//
// Run:
//
...
// Override target URL:
//
//	BASE_URL=http://localhost:7045 go test -tags=e2e -v ./e2e/
package e2e_test
